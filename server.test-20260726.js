    ...
    if (rel === 'index.html') {
      ...
      if (testimonial) {
        ...
        for (const [id,type,label,value] of virtualText) ext.blocks.push({
          ...
          orig:id,origVal:value,virtual:true,
        });
      }
...
app.patch('/api/admin/page-patch', requireAuth, (req, res) => {
  ...
  const prePairs = safeTextPairs((blocks || [])
    .filter(b => b && b.id && !b.isNew && !b.delete && b.newVal != null && b.origVal != null
                 && b.origVal !== b.newVal && b.type !== 'img'
                 && !b.id.startsWith('bf_home_testimonial_'))
    .map(b => ({ oldVal: b.origVal, newVal: b.newVal })));
  const preHtml = html;
  ...
  for (const b of blocks) {
    ...
    if (b.isNew) {
      ...
      else if (tag === 'a') newEl = $(`<a data-bf-id="${b.id}" href="${encodeHtmlEnts(sanitizeHref(b.newHref || ''))}" class="gp-btn-p" style="margin-top:10px">${b.newVal || ''}</a>`);
    ...
    if (b.newVal != null && b.origVal !== b.newVal) {
      if (b.type === 'img') el.attr('src', encodeHtmlEnts(b.newVal));
        ...
        if (/^bf_stg\d+_(title|label|desc)$/.test(b.id)) {
          stageEdits.push({ id: b.id, newVal: cleanText(b.newVal) });
        } else {
          textPairs.push({ oldVal: b.origVal, newVal: b.newVal });
        }
      }
      changed++;
    }
    ...
    if (stageEdits.length) {
      ...
      if (row) {
        ...
        if (stageChanged) {
          ...
          gUpd.run({ ...gameBody({ ...current, stages }, row), id: row.id });
...
const RUNTIME_JS = `(function(){
  ...
  function apply(s){try{
    ...
    applyTestimonialImage(hp.testimonialImage||'');
    setTimeout(function(){applyTestimonialImage(hp.testimonialImage||'');},300);
    setTimeout(function(){applyTestimonialImage(hp.testimonialImage||'');},1200);
  }catch(e){}}
  function applyTestimonialImage(url){
    ...
    img.id='bf-testimonial-image';img.src=url;img.alt='Фото автора відгуку';
    img.style.cssText='display:block;width:96px;height:96px;object-fit:cover;border-radius:50%;margin:0 auto 22px;border:3px solid rgba(0,159,227,.18);box-shadow:0 10px 30px rgba(15,23,42,.14);position:relative;z-index:10;background:#fff';
    if(img.parentElement!==card)card.insertBefore(img,quote);
  }
