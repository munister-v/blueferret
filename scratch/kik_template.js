...
function generateKikHtml(projects) {
  ...
  for (const e of projects) {
    if (e.status === 'draft') continue; // Don't show drafts publically
    ...
    html += `
        <a href="${e.campaign_url || '#'}" target="${e.campaign_url ? '_blank' : '_self'}" class="group flex h-full flex-col bg-white/98 rounded-2xl overflow-hidden board-game-border-kik transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_26px_46px_-30px_rgba(15,23,42,0.45)] bf-reveal">
          <div class="aspect-[16/10] min-h-[190px] relative overflow-hidden bg-gradient-to-br from-emerald-800/95 via-teal-900/90 to-slate-900">
            ${e.cover_url ? `<img src="${e.cover_url}" alt="${e.title}" class="absolute inset-0 w-full h-full object-cover">` : ''}
...
// Test with dummy data similar to the DB structure
const mockProjects = [
  {
    id: 1, title: 'Місто таємниць', description: 'Детективна гра для 2–6 гравців.',
    status: 'active', goal: 120000, raised: 67500,
    cover_url: '/images/placeholder-project.svg',
    campaign_url: '/kik/proekty/1'
  },
  {
    id: 3, title: 'Казковий ліс', description: 'Кооперативна гра для родини.',
    status: 'preparing', goal: 95000, raised: 0,
    cover_url: '/images/placeholder-project.svg',
    campaign_url: '/kik/proekty/3'
  }
];

console.log(generateKikHtml(mockProjects));
